...
print_inline(:(A::B))
print_inline(:(print_expr(:(function foo(x, y::T; z::Int = 1) where {N, T <: Real}; x + 1; end))))
...
print_inline(:(foo() do; end))
print_inline(:(foo() do x; end))
print_inline(:(foo() do x, y, z; end))
print_inline(:(foo() do x, y, z; 1+1;2+2; end))
print_inline(:(foo() do x, y, z...; 1+1;2+2; end))
...
print_inline(:([[line] for line in eachsplit(ex, '\n')]))
print_inline(:(Any[[line] for line in eachsplit(ex, '\n')]))

print_inline(quote; x+1; y+1; end)
print_inline(@expr :(using $(&name)))
print_inline(Expr(:string, "aaa", :x, "bbb"))

print_inline(:(:x))
print_inline(:(:(1 + 1; 2 + x)))
print_inline(:(1 + 1; 2 + x))
print_inline(:(let x = 1; y+1 end))
